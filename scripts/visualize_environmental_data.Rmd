...
plot_ts_direct <- function(df_name, var_name, return_plot=T){
  ...
  sdf <- df_name %>% #as_tibble(xy=T) %>% 
    ...
    summarise(m2 = mean(mean_bt_gfdl))
      #m=mean(.data[[var_name]],na.rm=T)) %>% # this is producing a value that does not change between 1980-2100
    ungroup()
  if(return_plot){
    # create labels for latitude bins
    sdf <- sdf %>% 
...
```{r}
purrr::map(list(df_summ), plot_ts_direct) # i need to fix this line so it reads all arguments of the plot_ts_direct function
```
