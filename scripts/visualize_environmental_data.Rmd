...
purrr::map(model_var, plot_ts_direct, df_name = df_summ, return_plot=T) 
```

And save a tibble

```{r}
latbins_summ <- purrr::map(model_var, plot_ts_direct, df_name = df_summ, return_plot=F)
...
plot_map_direct <- function(df_name, var_name, return_plot=T){
  ...
  if(return_plot){
    ...
    p_delta <- df_delta %>% 
      ...
      theme_classic() + labs(y="", x="", title = pt, subtitle = "Difference between 2070-2100 and 1980-2010") + 
      theme(legend.position="right",legend.title = element_blank()) +
      theme( panel.border = element_rect(colour = "black", fill=NA, size=1)) + #makes a box
...
purrr::map(model_var, plot_map_direct, df_name = df_summ, return_plot=T)
```

And save a tibble

```{r}
maps_df <- purrr::map(model_var, plot_map_direct, df_name = df_summ, return_plot=F)
