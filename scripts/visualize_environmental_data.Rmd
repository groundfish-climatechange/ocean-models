...
Start by looking at 1980-2010 vs 2070-2100
Then make a difference map
```{r}
df_name <- df_summ
var_name <- 'mean_bt_gfdl'
...
plot_map_direct <- function(df_name, var_name, return_plot=T){
  ...
  if(return_plot){
    ...
    p_begin <- df_begin %>% 
      ...
      geom_tile(aes(fill=m.y)) +
      theme_classic() + labs(y="", x="", title = pt) + 
      theme(legend.position="right",legend.title = element_blank()) +
      theme( panel.border = element_rect(colour = "black", fill=NA, size=1)) + #makes a box
      scale_fill_gradientn(colours = cmocean("ice")(256), name=lab) +
